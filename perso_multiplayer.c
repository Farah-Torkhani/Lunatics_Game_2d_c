...
void jumpAnimation2(Hero *h, Hero *h2, SDL_Surface *screen, GameplayBg *b, enemie f, int run, Input I)
{
    int i;
    if (h->currentMode == 4 && I.jump == 1)
    {
        for (i = 40; i < 50; i++)
        {
            ...
            afficher_Hero(h2, screen);

            update_ennemi(&f, screen, h->heroPos, *b, h);
...
void attackAnimation2(Hero *h, Hero *h2, SDL_Surface *screen, GameplayBg *b, enemie f, Input I)
{
    int i;
...
void leftAndRightHeroMvtR(Hero *hero, Input I, Uint32 dt)
{
    ...
    {
        if (I.right == 1 && (hero->heroPos.x <= 180 || (hero->heroPos_relative.x >= 9500 && hero->heroPos.x <= 1000)))
...
void deadAnimation2(Hero *h, Hero *h2, SDL_Surface *screen, GameplayBg *b, enemie f)
{
    int i;
